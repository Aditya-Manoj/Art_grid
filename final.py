...
def main():
    ...
    c_tuple = (255, 255, 255)
    gthick = int(input("ENTER the THICKNESS of grid lines (DEFAULT 5): "))
    if color in ('B', 'b', 'Black', 'BLACK'):
        c_tuple = (0, 0, 0)
    while True:
        if height >= hcount >= 0:
            cv2.line(pic, (0, hcount), (width, hcount), c_tuple, gthick)
            hcount += u_height
        else:
            break
    while True:
        if width >= wcount >= 0:
            cv2.line(pic, (wcount, 0), (wcount, height), c_tuple, gthick)
            wcount += u_width
        else:
            break
